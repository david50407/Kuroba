<div class="msg-detail" id="msg-<?= $_->name ?>-new">
	<form action="board-<?= $_->name?>/message-<?= $_->thread['id'] ?>/reply-<?= $_->reply['id'] ?>/edit" method="post" class="msg-form" data-pjax>
...
<?php endif; ?>
	...
	<div class="msg-detail-reply-box pure-u-1">
		<div class="msg-detail-reply">
			<div class="msg-detail-header pure-g">
				<div class="pure-u"><img src="http://www.gravatar.com/avatar/<?= md5(strtolower($_->authorR['email'])) ?>?d=<?= urlencode("http://placehold.it/64.png") ?>&s=64" alt="" class="avatar" /></div>
				<div class="pure-u-1 msg-detail-header-content">
					<h1 class="msg-detail-title">Re: <?= $_->reply['title'] ?>
...
<?php endif; ?>
					...
					<p class="msg-detail-subtitle">From <a href="mailto:<?= $_->authorR['email'] ?>" target="_blank"><?= $_->authorR['username'] ?></a> at <span><?= date("g:ia, F j, Y", strtotime($_->reply['created_at'])) ?></span></p>
				</div>
			</div>
			<div class="msg-detail-body">
